...
ImGuiLayer::ImGuiLayer() : Layer("ImGuiLayer"), m_Time(0.0) { }
...
// An ImGuiLayer receives an event when the OnEvent() function is called. The received event is
// compared against a list of events and gets dispatched to the event handler of matching type.
...
void ImGuiLayer::OnEvent(Event &event)
{
    ...
    dispatcher.Dispatch<KeyCharEvent>(HZ_BIND_EVENT_FN(ImGuiLayer::OnKeyCharEvent));
...
{
    ImGuiIO &io = ImGui::GetIO();
    ImGuiKey key = KeyToImGuiKey(event.GetKeyCode());
...
{
    ImGuiIO &io = ImGui::GetIO();
    io.AddKeyEvent(KeyToImGuiKey(event.GetKeyCode()), false);
    return false;
}

bool ImGuiLayer::OnKeyCharEvent(KeyCharEvent &event)
{
    ImGuiIO &io = ImGui::GetIO();
